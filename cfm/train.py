...
from cfm.utils.parser import ModelConfig, PEFTLoraConfig, TrainingConfig, DataConfig
from cfm.utils.training_utils import load_model_from_checkpoint, find_target_linear_names
from cfm.utils.parser import parse_args_with_yaml
...
def train(config, local_rank=0, debug=False):
    if local_rank == 0:
        wandb.init(project=os.getenv("WANDB_PROJECT", "cfm"), name=os.getenv("WANDB_RUN_NAME", None))
    ...
    test_dataset = PairwiseOriginalDataset(
        data_config.test_json_list,
        data_config.soft_label,
        data_config.confidence_threshold,
    )
    ...
    trainer = VLMRewardTrainer(
        ...
        eval_dataset=(test_dataset if training_args.conduct_eval else None),
