...
import wandb
from cfm.utils.training_utils import get_peft_state_non_lora_maybe_zero_3
...
class Qwen2VLRewardModelBT(Qwen2VLForConditionalGeneration):
    ...
    def forward(
        ...
        return {"logits": pooled_logits, "hidden_states": outputs['hidden_states']}
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if self.loss_type == "uncertainty":
            ...
            if (hasattr(self.state, 'global_step') and 
                ...
                self.state.global_step > 0) and self.args.process_index == 0:
        ...
        try:
            ...
            for i in range(batch_size):
                ...
                if writer is not None:
                    writer.add_scalar(f'train_viz/sample_{i}/score_A', score_A_val, self.state.global_step)
                    writer.add_scalar(f'train_viz/sample_{i}/score_B', score_B_val, self.state.global_step)
                    writer.add_scalar(f'train_viz/sample_{i}/score_diff', score_diff, self.state.global_step)
                else:
                    # Use wandb
                    wandb.log({
                        f'train_viz/sample_{i}/score_A': score_A_val,
                        f'train_viz/sample_{i}/score_B': score_B_val,
                        f'train_viz/sample_{i}/score_diff': score_diff,
                        'step': self.state.global_step
                    })
                try:
                    ...
                    if writer is not None:
                        writer.add_figure(f'train_viz/sample_{i}_comparison', fig, self.state.global_step)
                    else:
                        wandb.log({f'train_viz/sample_{i}_comparison': wandb.Image(fig), 'step': self.state.global_step})
                    plt.close(fig)
            ...
            if writer is not None:
                writer.add_histogram('train_viz/all_scores_A', all_scores_A, self.state.global_step)
                writer.add_histogram('train_viz/all_scores_B', all_scores_B, self.state.global_step)
                writer.add_scalar('train_viz/mean_score_A', float(all_scores_A.mean()), self.state.global_step)
                writer.add_scalar('train_viz/mean_score_B', float(all_scores_B.mean()), self.state.global_step)
                writer.add_scalar('train_viz/mean_score_diff', float((all_scores_A - all_scores_B).mean()), self.state.global_step)
            else:
                wandb.log({
                    'train_viz/all_scores_A': wandb.Histogram(all_scores_A),
                    'train_viz/all_scores_B': wandb.Histogram(all_scores_B),
                    'train_viz/mean_score_A': float(all_scores_A.mean()),
                    'train_viz/mean_score_B': float(all_scores_B.mean()),
                    'train_viz/mean_score_diff': float((all_scores_A - all_scores_B).mean()),
                }, step=self.state.global_step)
        except Exception as e:
            print(f"On batch{i},Error in training visualization: {e}")
